...
class _BangumiMirrorInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    // Subject search on the official mirror is signature-protected
    // (KAZUMI_APPID, injected by upstream CI only). Builds without it fall
    // back to a community v0 mirror so search keeps working; everything else
    // on the mirror is open.
    if (!hasBangumiMirrorCredentials &&
        options.method == 'POST' &&
        uri.path == '/v0/search/subjects') {
      KazumiLogger().d(
          'Bangumi search mirror: ${ApiEndpoints.bangumiSearchMirrorDomain}');
      options.path = ApiEndpoints.bangumiSearchMirrorDomain +
          uri.path +
