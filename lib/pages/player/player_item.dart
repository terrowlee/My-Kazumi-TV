...
class _PlayerItemState extends State<PlayerItem>
  ...
  void _initKeyboardActions() {
    keyboardActions = {
      ...
      'rewind': () async {
        if (TvMode.enabled) {
          await handleShortcutRewindDown();
        } else {
          await handleShortcutRewind();
        }
      },
    ...
    keyboardLongPressActions = {
      ...
      'rewind': PlayerLongPressShortcutActions(
        onRepeat: handleShortcutRewindRepeat,
        onRelease: handleShortcutRewindUp,
      ),
    };
  }
  ...
  // TV remote: hold LEFT/RIGHT accumulates an interactive seek (MyTVB-style
  // SeekSession) with the seek HUD visible, and the position only commits on
  // key release. Short press still seeks one arrowKeySkipTime step because a
  // KeyDown without a following KeyRepeat falls through to the plain seek.
  int? _tvSeekDirection;

  Future<void> _beginTvSeekSession(int direction) async {
    if (_tvSeekDirection != null) return;
    _tvSeekDirection = direction;
    playerController.panel.seekDirection = direction;
    playerController.panel.showSeekTime = true;
    _beginInteractiveSeek();
    _extendTvSeekSession(direction);
  }

  void _extendTvSeekSession(int direction) {
    if (_tvSeekDirection == null) return;
    playerController.seeking.updateInteractiveSeek(
      playerController.playback.currentPosition +
          Duration(
            seconds: playerController.playback.arrowKeySkipTime * direction,
          ),
    );
  }

  Future<void> _endTvSeekSession() async {
    final hadSession = _tvSeekDirection != null;
    _tvSeekDirection = null;
  ...
  void handleShortcutForwardDown() {
    if (TvMode.enabled) {
      unawaited(_beginTvSeekSession(1));
      return;
    }
