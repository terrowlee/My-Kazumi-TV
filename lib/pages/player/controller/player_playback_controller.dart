...
abstract class _PlayerPlaybackController with Store {
  ...
  Future<Player?> createVideoController(
    ...
    try {
      ...
      if (Platform.isAndroid) {
        ...
        // Software decode skips high-quality decoder features; 32-bit TV
        // CPUs need the headroom. No effect when hardware decode is active.
      ...
      videoController ??= VideoController(
        ...
        configuration: VideoControllerConfiguration(
          ...
          // SurfaceProducer (AImageReader/HardwareBuffer) composites video
          // frames far cheaper than the SurfaceTexture path on API 29+;
          // opt-in via 设置→视频渲染器 since upstream defaults it off.
          enableAndroidSurfaceProducer: Platform.isAndroid &&
  ...
  Future<void> playOrPause({
    ...
    if (playerPlaying) {
      await pause();
      // Progress is only persisted on pause/exit/episode-switch (TV storage:
      // per-second Hive writes caused periodic UI freezes), so save here.
      await recordHistory();
    } else {
      await play();
    }
