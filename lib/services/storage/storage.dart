...
class GStorage {
  ...
  static Future init() async {
    ...
    histories = await _openBoxSafe<History>('histories',
        compactionStrategy: _heavyChurnCompaction);
  ...
  /// Boxes written every few seconds during playback (history progress,
  /// sync-change appends, setting sliders) trigger Hive compaction often;
  /// each compaction rewrites the whole file and freezes the UI for seconds
  /// on slow TV flash storage. Require far more churn before compacting.
  static bool _heavyChurnCompaction(int entries, int deletedEntries) {
    return deletedEntries > 400;
  }

  static bool _defaultCompaction(int entries, int deletedEntries) {
    return deletedEntries > 60;
  }

  /// Open a Hive box with automatic recovery on corruption.
  /// If the box is corrupted, delete it and create a new empty one.
  static Future<Box<T>> _openBoxSafe<T>(String boxName,
      {CompactionStrategy? compactionStrategy}) async {
    try {
      return await Hive.openBox<T>(boxName,
          compactionStrategy:
              compactionStrategy ?? _defaultCompaction);
    } catch (e) {
      KazumiLogger().e(
