...
class SettingsKeys {
  ...
  /// 弹弹Play兼容弹幕服务器地址。空 = 官方 api.dandanplay.net（需要签名凭证）。
  /// 指向自建/第三方兼容服务（如 danmu_api）时可无需凭证。
  static const danmakuApiBaseUrl = SettingKey<String>(
    'danmakuApiBaseUrl',
    '',
    group: SettingGroup.danmaku,
  );
  ...
  /// Flutter SurfaceProducer 合成管线（API 29+ 走 AImageReader/HardwareBuffer，
  /// 每帧合成开销低于 SurfaceTexture）。实验开关：默认关闭保持上游行为。
  static const androidSurfaceProducer = SettingKey<bool>(
    'androidSurfaceProducer',
    false,
    group: SettingGroup.player,
  );
