...
- 播放中**长按左/右键 = 快进快退会话**：HUD 实时预览目标位置，松开才跳转；短按 ±10 秒
- 首页支持数字键直达节目（1–999）
- 部分电视系统用户空间为 32 位，提供 `armeabi-v7a` 包

### 设置页重构（针对电视操作）

- 设置**嵌入主边栏**：点「设置」后左侧主边栏保留，左侧设置分类 + 右侧详情双栏
- 所有设置行聚焦时显示高对比边框，多选一设置改为**弹出选择**
- 二级/三级页面全部在内嵌布局中打开，返回键：详情 → 分类列表 → 主页
- 移除触摸时代的多余元素：卡片底壳、返回/关闭按钮、画中画、全屏按钮
...
- **二维码输入**：搜索框与设置对话框旁显示二维码，手机扫码打开局域网页面直接打字，
  回传到电视自动执行（无需任何外部服务）
- **整体缩放**：界面设置 → 显示大小，75%–150% 即时生效，且不影响视频播放画面
- TV 端移除触摸滑动手势等
