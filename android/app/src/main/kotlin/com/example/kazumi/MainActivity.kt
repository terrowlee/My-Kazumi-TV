...
class MainActivity: AudioServiceActivity() {
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        if (isTelevision()) {
            // Some TV ROMs report system-navigation compat keycodes (280-283)
            // instead of DPAD events. Translate once at the activity boundary
            // so focus traversal, shortcuts and seek handling all see
            // standard DPAD keys.
            val compatKeyCode = romNavigationCompatKeyCode(event.keyCode)
            if (compatKeyCode != null) {
                return super.dispatchKeyEvent(
                    KeyEvent(
                        event.downTime, event.eventTime, event.action, compatKeyCode,
    ...
    private fun romNavigationCompatKeyCode(keyCode: Int): Int? = when (keyCode) {
        280 -> KeyEvent.KEYCODE_DPAD_UP
        281 -> KeyEvent.KEYCODE_DPAD_DOWN
        282 -> KeyEvent.KEYCODE_DPAD_LEFT
        283 -> KeyEvent.KEYCODE_DPAD_RIGHT
        else -> null
    }
